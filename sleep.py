def sleep():
    ...
    def draw():
        screen.blit(
            sleep_bg,
            (
                (window_width - sleep_bg.get_width()) / 2,
                (window_height - sleep_bg.get_height()) / 2,
            ),
        )
        ...
        screen.blit(
            sleeptext1,
            (
                (window_width - sleeptext1.get_width()) / 2,
                (window_height - sleeptext1.get_height()) / 3 - 10 * k,
            ),
        )
        screen.blit(
            sleeptext2,
            (
                (window_width - sleeptext2.get_width()) / 2,
                (window_height - sleeptext2.get_height()) / 3
                + sleeptext1.get_height()
                + 5 * k,
            ),
        )
        screen.blit(
            adtext,
            (
                (window_width - adtext.get_width()) / 2,
                (window_height - adtext.get_height()) - 5 * k,
            ),
        )
        pygame.display.flip()
    ...
    def scale_bg():
        ...
        ratio = max(
            window_width / sleep_bg_original.get_width(),
            window_height / sleep_bg_original.get_height(),
        )
        bg = pygame.transform.smoothscale(
            sleep_bg_original,
            (
                int(sleep_bg_original.get_width() * ratio),
                int(sleep_bg_original.get_height() * ratio),
            ),
        )
        sleep_bg = bg
    ...
    if random_sleep_bg:
        globals()['sleep_bg_path'] = get_random_image_from_directory(
            sleep_bg_directory
        )
    ...
    globals()['sleep_bg_original'] = pygame.image.load(
        sleep_bg_path
    ).convert_alpha()
    scale_bg()
