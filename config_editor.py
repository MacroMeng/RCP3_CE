...
class DropRateEditor(QDialog):
    ...
    def load_data(self):
        ...
        for row, (name, final_rate) in enumerate(sorted(all_rates.items())):
            ...
            auto_rate_item.setFlags(
                auto_rate_item.flags() & ~Qt.ItemIsEditable
            )
    ...
    def view_history(self):
        ...
        sorted_records = sorted(
            history_records, key=lambda x: x.get('timestamp', ''), reverse=True
        )
    ...
    def reset_today_history(self):
        """重置当天历史记录"""
        reply = QMessageBox.question(
            self, '确认重置', '确定要重置今天的历史记录吗？', QMessageBox.Yes | QMessageBox.No
        )
        if reply == QMessageBox.Yes:
        ...
        try:
            ...
            for row in range(self.table.rowCount()):
                ...
                try:
                    ...
                    if rate < 0 or rate > 1:
                        QMessageBox.warning(
                            self, '错误', f'爆率必须在 0.0 到 1.0 之间: {name}'
                        )
                        return

                    # 如果是手动配置，保存到手动文件
                    if source == '手动':
                        self.drop_rate_manager.set_drop_rate(
                            name, rate, is_manual=True
                        )
                    # 如果是自动配置且被修改过，转为手动配置
...
class ConfigEditor(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.reload_btn.setStyleSheet(
            'QPushButton { background-color: #2196F3; color: white; font-size: 14px; padding: 8px; }'
        )

        self.reset_btn = QPushButton('重置为默认')
        self.reset_btn.clicked.connect(self.reset_to_default)
        self.reset_btn.setStyleSheet(
            'QPushButton { background-color: #ff9800; color: white; font-size: 14px; padding: 8px; }'
        )
    ...
    def create_basic_tab(self):
        ...
        self.init_bg_btn.clicked.connect(
            lambda: self.browse_file(self.init_bg_edit)
        )
        ...
        self.main_bg_btn.clicked.connect(
            lambda: self.browse_file(self.main_bg_edit)
        )
        ...
        self.init_bg_dir_btn.clicked.connect(
            lambda: self.browse_directory(self.init_bg_dir_edit)
        )
        ...
        self.main_bg_dir_btn.clicked.connect(
            lambda: self.browse_directory(self.main_bg_dir_edit)
        )
    ...
    def create_sleep_tab(self):
        ...
        self.sleep_bg_dir_btn.clicked.connect(
            lambda: self.browse_directory(self.sleep_bg_dir_edit)
        )
    ...
    def create_developer_tab(self):
        ...
        self.icon_path_btn.clicked.connect(
            lambda: self.browse_file(self.icon_path_edit)
        )
    ...
    def load_config(self):
        ...
        try:
            ...
            try:
                ...
                self.load_config_to_ui()
                QMessageBox.warning(
                    self, '编码提示', '配置文件使用GBK编码，已成功加载。保存时将转换为UTF-8编码。'
                )
            except:
                # 如果都失败，创建默认配置
    ...
    def load_config_to_ui(self):
        ...
        self.language_combo.setCurrentText(
            self.config.get('Language', 'defult_language', fallback='Chinese')
        )
        self.start_pos_check.setChecked(
            self.config.getboolean(
                'Basic', 'startwindowpositioncontrol', fallback=False
            )
        )
        self.start_pos_edit.setText(
            self.config.get('Basic', 'startwindowposition', fallback='0,0')
        )
        self.message_time_spin.setValue(
            self.config.getint('Basic', 'message_time_length', fallback=5)
        )
        self.init_bg_edit.setText(
            self.config.get(
                'Basic',
                'init_background',
                fallback='./images/backgrounds/--background.png',
            )
        )
        self.main_bg_edit.setText(
            self.config.get(
                'Basic',
                'background_img',
                fallback='./images/backgrounds/background.png',
        ...
        self.random_bg_check.setChecked(
            self.config.getboolean('Basic', 'use_random_bg', fallback=True)
        )
        self.init_bg_dir_edit.setText(
            self.config.get(
                'Basic',
                'init_bg_directory',
                fallback='./images/backgrounds/init_bg',
            )
        )
        self.main_bg_dir_edit.setText(
            self.config.get(
                'Basic',
                'main_bg_directory',
                fallback='./images/backgrounds/main_bg',
            )
        )

        # 休眠设置
        self.sleep_alpha_spin.setValue(
            self.config.getint('sleep', 'sleep_background_alpha', fallback=190)
        )
        self.sleep_bg_dir_edit.setText(
            self.config.get(
                'sleep',
                'sleep_bg_directory',
                fallback='./images/backgrounds/init_bg',
        ...
        self.random_sleep_bg_check.setChecked(
            self.config.getboolean('sleep', 'random_sleep_bg', fallback=True)
        )
        self.touch_delay_spin.setValue(
            self.config.getfloat('sleep', 'touch_delay', fallback=1.0)
        )
        self.sleep_delay_spin.setValue(
            self.config.getint('sleep', 'sleep_time_delay', fallback=30)
        )
        self.sleep_fps_spin.setValue(
            self.config.getint('sleep', 'sleepfps', fallback=60)
        )

        # 高级设置
        self.screen_size_edit.setText(
            self.config.get('Advanced', 'screensize', fallback='(960,540)')
        )
        self.init_alpha_spin.setValue(
            self.config.getint(
                'Advanced', 'init_background_alpha', fallback=175
            )
        )

        # 性能设置
        self.animation_check.setChecked(
            self.config.getboolean('Xingneng', 'animation', fallback=True)
        )
        self.animation_speed_spin.setValue(
            self.config.getint('Xingneng', 'animationspeed', fallback=5)
        )
        self.init_fps_spin.setValue(
            self.config.getint('Xingneng', 'init_fps', fallback=120)
        )
        self.main_fps_spin.setValue(
            self.config.getint('main_settings', 'fps', fallback=60)
        )
        self.bar_speed_spin.setValue(
            self.config.getint('main_settings', 'barspeed', fallback=5)
        )

        # 点名设置
        self.name_file_edit.setText(
            self.config.get('chooser', 'name_text', fallback='name.txt')
        )
        self.except_file_edit.setText(
            self.config.get(
                'chooser', 'except_names', fallback='name_except.txt'
            )
        )
        self.drop_rate_file_edit.setText(
            self.config.get(
                'chooser', 'drop_rate_file', fallback='drop_rates.json'
            )
        )

        mode = self.config.get(
            'chooser', 'choose_mode', fallback='smart_balance'
        )
        mode_index = {
            'repeat': 0,
            'single_no_repeat': 1,
        ...
        # 新增：手动爆率覆盖选项
        self.use_manual_override_check.setChecked(
            self.config.getboolean(
                'chooser', 'use_manual_override', fallback=True
            )
        )

        self.balance_weight_spin.setValue(
            self.config.getfloat('chooser', 'balance_weight', fallback=0.7)
        )
        self.smart_sensitivity_spin.setValue(
            self.config.getfloat('chooser', 'smart_sensitivity', fallback=0.5)
        )
        self.enable_drop_rate_check.setChecked(
            self.config.getboolean(
                'chooser', 'enable_drop_rate', fallback=False
            )
        )

        # 开发者设置
        self.icon_path_edit.setText(
            self.config.get('Developer', 'MHicon', fallback='./images/14.ico')
        )
        self.font_edit.setText(
            self.config.get('Developer', 'Font', fallback='MicrosoftYaHei UI')
        )
        self.first_name_edit.setText(
            self.config.get('Developer', 'firstdraw_lastname', fallback='?')
        )
        self.welcome_msg_edit.setText(
            self.config.get(
                'Developer',
                'welcomemessage',
    ...
    def create_default_config(self):
        ...
        self.config['main_settings'] = {'fps': '60', 'barspeed': '5'}
        self.config['chooser'] = {
            'name_text': 'name.txt',
    ...
    def save_config(self):
        ...
        try:
            ...
            self.config['Basic'] = {
                'startwindowpositioncontrol': str(
                    self.start_pos_check.isChecked()
                ),
            ...
            self.config['chooser'] = {
                ...
                'use_manual_override': str(
                    self.use_manual_override_check.isChecked()
                ),  # 新增
                ...
                'enable_drop_rate': str(
                    self.enable_drop_rate_check.isChecked()
                ),
            }
    ...
    def reset_to_default(self):
        """重置为默认值"""
        reply = QMessageBox.question(
            self, '确认重置', '确定要重置所有设置为默认值吗？', QMessageBox.Yes | QMessageBox.No
        )
        if reply == QMessageBox.Yes:
            self.create_default_config()
            self.load_config_to_ui()
    ...
    def update_drop_rates(self):
        ...
        try:
            ...
            if not name_list:
                try:
                    with open(
                        name_file, 'r', encoding='utf-8', errors='ignore'
                    ) as f:
                        name_list = [
                            line.strip() for line in f if line.strip()
                        ]
            ...
            # 更新爆率配置
            updated, new_names = self.drop_rate_manager.update_from_list(
                name_list
            )
