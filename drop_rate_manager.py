...
class OptimizedDropRateManager:
    ...
    def recalculate_auto_rates(self):
        ...
        for record in self.history_records:
            ...
            # 更新首次和最后被点时间
            if name not in name_first_draw or record_date < name_first_draw[name]:
                name_first_draw[name] = record_date
            if name not in name_last_draw or record_date > name_last_draw[name]:
        ...
        for name in name_stats:
            if name in name_last_draw:
                name_stats[name]['last_draw_days'] = (now - name_last_draw[name]).days
            if name in name_first_draw:
                name_stats[name]['first_draw_days'] = (now - name_first_draw[name]).days
        ...
        for name, stats in name_stats.items():
            ...
            if total_count > 0 and last_draw_days > 14:  # 被点过但很久没点了
                long_time_no_see_bonus = min(0.4, 
                    (math.log(last_draw_days) * 0.15) + (math.log(total_count) * 0.05))
            ...
            # 最终爆率计算
            base_rate = 1.0  # 基础爆率
            calculated_rate = base_rate - base_penalty - recent_penalty + cool_down_bonus + newcomer_bonus + long_time_no_see_bonus + balance_factor
