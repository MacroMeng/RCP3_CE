...
def randomchoice(flush=True, record_history=True):
    ...
    if choose_mode == 'repeat':
        ...
        available_names = [
            n
            for n in name
            if choose_manager.get_choice_count(n, 'history_balance') == 0
        ]
        if available_names:
...
def weighted_random_choice(name_list):
    ...
    for name in name_list:
        drop_rate = drop_rate_manager.get_drop_rate(
            name, use_manual_override=use_manual_override
        )
...
def balanced_choice(name_list, mode):
    ...
    for name in name_list:
        if mode == 'today_balance':
            ...
            today_count = choose_manager.get_choice_count(
                name, 'today_balance'
            )
            history_count = choose_manager.get_choice_count(
            ...
            history_weight = (
                history_count * (1 - smart_sensitivity) * 0.3
            )  # 历史影响减弱
...
def test_balance_mechanism(test_rounds=1000):
    ...
    for mode in test_modes:
        ...
        std_dev = (
            sum((x - avg_picks) ** 2 for x in pick_counts) / len(pick_counts)
        ) ** 0.5
    ...
    if choose_mode in ['today_balance', 'history_balance', 'smart_balance']:
        ...
        for student in name[:5]:  # 只显示前5个作为样本
            if choose_mode == 'today_balance':
                count = choose_manager.get_choice_count(
                    student, 'today_balance'
                )
                weight = 1.0 / (1.0 + count * balance_weight * 2)
            elif choose_mode == 'history_balance':
                count = choose_manager.get_choice_count(
                    student, 'history_balance'
                )
