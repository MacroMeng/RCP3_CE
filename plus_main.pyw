    ...
    if not func_file:
        for function_file in important_function_list:
            try:
                # 强制使用 UTF-8 编码，忽略错误
                with open(
                    function_file, 'r', encoding='utf-8', errors='ignore'
                ) as f:
                    try:
                        file_content = f.read()
...
def find_and_activate_main_window(window_title):
    ...
    try:
        ...
        win32gui.RedrawWindow(
            hwnd, None, None, win32con.RDW_INVALIDATE | win32con.RDW_UPDATENOW
        )
        ...
        def force_foreground(hwnd):
            ...
            win32process.AttachThreadInput(
                window_thread, current_thread, False
            )
...
if not is_first:
    ...
    try:
        ...
        print('🔄 优先尝试管道通信...')
        success, response = send_pipe_message(
            r'\\.\pipe\FloatingButtonPipe', 'SHOW_WINDOW'
        )
...
def proportional_scale(img, max_width, max_height):
    ...
    ratio = max(
        window_width / img.get_width(), window_height / img.get_height()
    )
...
def background():
    ...
    try:
        screen.blit(
            bg,
            (
                (window_width - bg.get_width()) / 2,
                (window_height - bg.get_height()) / 2,
            ),
        )
    except:
        screen.fill((0, 0, 0))
...
def showclock(flush=True):
    ...
    screen.blit(
        clock_time,
        (
            (130 * k - clock_time.get_width()) / 2
            - 2 * k
            + (window_width - 130 * k),
            (40 * k - clock_time.get_height()) / 2,
        ),
    )
    if flush:
...
def draw_lastname(flush=True, temp=False):
    ...
    if temp:
        ...
        tempsurface_2.blit(
            namesurface,
            (
                (window_width - 4.5 * 150 * k) / 2 + 8 * k,
                (window_height - 150 * k) / 2 - 30 * k,
            ),
        )
        tempsurface_2.blit(
            text,
            (
                (4.5 * 150 * k + 30 * k - text.get_width()) / 2
                - 2 * k
                + (window_width - 4.5 * 150 * k) / 2
                + 8 * k,
                (150 * k + 60 * k - text.get_height()) / 2
                - 10 * k
                + (window_height - 150 * k) / 2
                - 30 * k,
            ),
        )
        return
...
def message(text, flush=True, temp=False):
    ...
    if text != '':
        ...
        pygame.draw.rect(
            messagesurface,
            (0, 0, 0, 127),
            (0, 0, _text_.get_width() + 20 * k, _text_.get_height() + 10 * k),
        ...
        updatearea = (
            (window_width - (_text_.get_width() + 20 * k)) / 2,
            60 * k,
            _text_.get_width() + 20 * k,
            _text_.get_height() + 10 * k,
        )
    else:
        messagesurface.fill((0, 0, 0, 0))
        screen.blit(messagesurface, pos)
