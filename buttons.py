...
def fullscreenbutton(style=0, flush=True, temp=False):
    ...
    if is_fullscreen:
        ...
        screen.blit(
            fullscreensurface, (window_width - 36 * k, window_height - 36 * k)
        )
        if flush:
            pygame.display.update(
                (window_width - 36 * k, window_height - 36 * k, 36 * k, 36 * k)
            )
    else:
        fullscreensurface.fill((0, 0, 0, 0))
        if style == 'click':
...
def draw_button(
    place, size, text, rad=3, color=(50, 50, 50), _alpha_=255, temp=False
):
