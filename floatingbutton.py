...
class FloatingButton(QPushButton):
    ...
    def start_message_listener(self):
        ...
        def message_loop():
            ...
            pipe_thread = threading.Thread(
                target=self.start_pipe_server, daemon=True
            )
            pipe_thread.start()
            print('✓ 管道服务器线程已启动')
            ...
            try:
                # 使用唯一的窗口类名
                class_name = (
                    f'FloatingButtonMessageWindow_{int(time.time()*1000)}'
                )
                ...
                while self.message_listening:
                    try:
                        # 处理窗口消息
                        result, msg = win32gui.PeekMessage(
                            self.message_hwnd, 0, 0, win32con.PM_REMOVE
                        )
                        if result:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        win32file.WriteFile(
                            pipe_handle, error_response.encode('utf-8')
                        )
                    except:
                        pass
                finally:
    ...
    def start_press_animation(self):
        ...
        self.press_animation.setEndValue(
            QSize(int(self.width() * 0.9), int(self.height() * 0.9))
        )
